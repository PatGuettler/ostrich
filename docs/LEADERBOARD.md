...
On Android, PGS handles platform authentication and opens Google's native all-users ranking screen. The game checks sign-in at startup and prompts for Google Play Games sign-in when the player taps **GLOBAL SCORES** if needed. A finished run's distance is queued when unsigned-in and submitted as soon as sign-in succeeds. On desktop, debug builds, or Android builds without IDs, the buttons show a friendly setup message and gameplay continues normally.
